...
CCompressor::~CCompressor() {
  if (options & DeleteLogger) delete logger;
  if (options & DeleteReader) delete BR;
  if (options & DeleteWriter) delete BW;
}
...
void CCompressor::compress() {
  try {
    ...
    if (options & UseLogger) {
      logger->log("ERROR! Compress failed: " + error_message);
    }
  }
}
...
void CCompressor::uncompress() {
  try {
    ...
    if (options & UseLogger) {
      logger->log("Uncompressed succesffully");
    }
  } catch(string error_message) {
    if (options & UseLogger) {
      logger->log("ERROR! Decompress failed: " + error_message);
    }
  }
}
